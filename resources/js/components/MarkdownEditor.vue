...
const insertTable = () => {
    editorRef.value?.editor?.chain().focus().insertTable({ rows: 2, cols: 2, withHeaderRow: true }).run();
};
...
router.on("success", (event) => {
    nextTick(() => {
        if (event?.detail?.page?.url.startsWith('/browser')) {
            layoutScrollArea.value?.scrollTo({ top: 0, left: 0, behavior: 'auto' });
        }
     })
});
</script>
    ...
    <div class="flex flex-col">
        <Teleport defer to="#browser-header-slot">
            <div class="flex flex-wrap items-center gap-2">
                <button
                    type="button"
                    class="rounded p-1 hover:!bg-neutral-100 dark:hover:!bg-neutral-700 hidden lg:block"
                ...
                <div class="h-6 w-px !bg-neutral-300 dark:!bg-neutral-600 hidden lg:block" />
                ...
                <button
                    type="button"
                    class="rounded p-1 line-through hover:!bg-neutral-100 dark:hover:!bg-neutral-700 hidden lg:block"
                ...
                <button
                    type="button"
                    class="rounded p-1 hover:!bg-neutral-100 dark:hover:!bg-neutral-700 hidden lg:block"
                    :class="{ '!bg-neutral-200 dark:!bg-neutral-600': isActive('table') }"
                    title="Table"
                    @click="insertTable"
                >
                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div ref="layoutScrollArea" class="editor-wrapper flex-1 overflow-auto pb-1">
            <NovelEditor
                v-show="mode === 'editor'"
                ref="editorRef"
            ...
            />
            <TableBubbleMenu :editor="editorRef?.editor" />
            <textarea
                v-show="mode === 'raw'"
                v-model="rawContent"
