...
Approach Pros Cons **Laravel + filesystem** Simple, portable, notes stay as files No real-time collab Laravel + database (store MD in DB) Faster search, history Breaks plain-file portability Static site (no backend) Ultra simple No workspace switching, no server-side path security
